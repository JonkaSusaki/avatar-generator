...
function rand(seed: Buffer, index: number) {
  return seed[index % seed.length] / 255;
}

async function returnBuffer(img: PImage.Bitmap) {
...
function rgb(seed: Buffer, i: number): string {
  const index = Math.floor(rand(seed, i) * palette.length);
  return palette[index];
}

function applyAlphaToColor(rgb: string, alpha: number): string {
...
function drawShape(
  ...
  ctx.translate(x, y);
  ctx.rotate(rotation);
  if (type === "square") drawGlowingSquare(ctx, 0, 0, size, color);
  else if (type === "triangle") drawGlowingTriangle(ctx, 0, 0, size, color);
  else if (type === "diamond") drawGlowingDiamond(ctx, 0, 0, size, color);
  ctx.restore();
  if (type === "square") drawGlowingSquare(ctx, x, y, size, color);
  else if (type === "triangle") drawGlowingTriangle(ctx, x, y, size, color);
  else if (type === "diamond") drawGlowingDiamond(ctx, x, y, size, color);
}
...
export async function generateAvatar(input: string, width = 200, height = 200) {
